...
export default function Home() {
  ...
  const sendFrame = async () => {
    if (snapshot) {
      ...
      setChatResponse("You need to send a snapshot too!")
    }
  }
  ...
  const handleResponse = async (data) => {
    ...
    imgFormData.append("profile", data.flaskResponse.profile);
    ...
    <div className="grid grid-rows-[20px_1fr_20px] items-start justify-items-center min-h-screen p-8 pb-20 gap-16 sm:p-20 font-[family-name:var(--font-geist-sans)]">
      ...
      <main className="flex flex-col gap-8 row-start-2 md:items-start max-w-screen-lg w-full space-y-5">
        ...
        {imgLoading && (
            <div className="w-full border p-4 rounded">
              {/* Example content */}
              <h2 className="text-lg font-bold animate-pulse">🎨 Generating a true canon photo of your OC... 🎨</h2>
            </div>
          )}
          {!imgLoading && genImg &&
            <div className="flex flex-col w-full">
        ...
        <div className="flex flex-col w-full space-y-5">
          ...
          {chatLoading && (
            <div className="w-full border p-4 rounded">
              {/* Example content */}
              <h2 className="text-lg font-bold animate-pulse">🤔 Thinking of a response... 🤔</h2>
            </div>
          )}
        </div>
